...
function CreatorDashboardPage() {
  ...
  const createPost = async (event) => {
    ...
    setPostErrors({})
    try {
      const cleanPost = await createPostSchema.validate({ text: postForm.text, hasImage: Boolean(postForm.image) }, { abortEarly: false })
      const formData = new FormData()
      if (cleanPost.text) formData.append('text', cleanPost.text)
        ...
        <Col lg={4}>
          <Card><Card.Body>
            ...
            <Form onSubmit={createPost}>
              ...
              <Form.Control className="mb-2" type="file" accept="image/*" isInvalid={Boolean(postErrors.form)} onChange={(e) => setPostForm((prev) => ({ ...prev, image: e.target.files?.[0] || null }))} />
              <Form.Control.Feedback type="invalid">{postErrors.form}</Form.Control.Feedback>
      ...
      <ListGroup>
        {dashboard.map((post) => (
          <ListGroup.Item key={post.id}>
            ...
            {post.image_url && <img src={post.image_url.startsWith('http') ? post.image_url : `http://localhost:3000${post.image_url}`} alt="post" className="img-fluid rounded d-block mt-2 mb-2" />}
